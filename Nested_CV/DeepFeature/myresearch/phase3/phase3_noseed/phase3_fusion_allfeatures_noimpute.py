#!/usr/bin/env python
# -*- coding: utf-8 -*-
# phase3_fusion_allfeatures_noimpute.py
# Frozen image head + Clinical residual
# Clinical = Age + Gender + T + N + M + OverallStage + Histology(4 one-hot + missing) => 11-dim
# 
# Step 3 Improvement: NO mode imputation for missing histology
# - Missing samples: all 4 one-hot = 0, missing_flag = 1
# - Non-missing: corresponding one-hot = 1, missing_flag = 0
...
def build_clinical_features(
    ...
    print(f"[Histology] Encoding: NO mode imputation (missing → all one-hot=0)")
    print()

    # ===== Step 3 Improvement: No mode imputation =====
    # For missing samples: all 4 one-hot = 0, missing_flag = 1
    # For non-missing: corresponding one-hot = 1, missing_flag = 0
    # This avoids forcing missing samples into a specific category (reduces noise)
    
    cats = ["adeno", "squamous", "large", "nos"]
    for cat in cats:
        # If missing (None), set to 0; if matches cat, set to 1; otherwise 0
        feats_tr.append(np.array([1.0 if (x is not None and x == cat) else 0.0 for x in h_tr_norm], dtype=np.float32))
        feats_va.append(np.array([1.0 if (x is not None and x == cat) else 0.0 for x in h_va_norm], dtype=np.float32))
...
def main():
    ...
    print("\n" + "=" * 60)
    print("SUMMARY (Fusion, 11-dim, NO histology imputation)")
    print("=" * 60)
