#!/usr/bin/env python
# -*- coding: utf-8 -*-
# phase3_clinical_age_gender_TNM_stage.py
# Clinical-only baseline: Age + Gender + T + N + M + Overall Stage (6-dim)
# M: binarized (M0→0, M≥1→1)
# Overall stage: ordinal group (I/II/III/IV → 1/2/3/4) + z-score
...
def _map_overall_stage_to_group(s) -> float:
    """
    将 overall stage (I/IA/IB/II/IIIA/IV 等) 映射到 1~4 的 ordinal：
    - I*   → 1
    - II*  → 2
    - III* → 3
    - IV*  → 4
...
def load_clinical_data(csv_path: str) -> pd.DataFrame:
    ...
    - T/N/M 用 pd.to_numeric 解析
    - overall_stage 用 _map_overall_stage_to_group 解析
    """
    ...
    # overall_stage → ordinal
    if "overall_stage" in df.columns:
        df["overall_stage"] = df["overall_stage"].apply(_map_overall_stage_to_group)
        print(
            "[Clinical] Overall stage valid:"
            f" {df['overall_stage'].notna().sum()} / {len(df)}"
        )
        print(
            f"[Clinical] Overall stage distribution: "
            f"{df['overall_stage'].value_counts().sort_index().to_dict()}"
        )
    else:
...
def build_clinical_features(
    ...
    print(
        f"[Features] M stage binarized: "
        f"train M1+={m_tr.sum():.0f}/{len(m_tr)}, "
    ...
    st_tr = (tr["overall_stage"].values - st_mu) / st_std
    st_va = (va["overall_stage"].values - st_mu) / st_std

    feats_tr.append(st_tr)
    feats_va.append(st_va)
...
def main():
    ...
    parser.add_argument("--exp_dir", type=str, required=True,
                        help="Phase3 image-only 实验目录（用于读取 cuts / folds）")
    parser.add_argument("--clinical_csv", type=str, required=True,
                        help="NSCLC-Radiomics-Lung1 临床 CSV 路径")
    ...
    print("\n" + "=" * 60)
    print("CLINICAL-ONLY BASELINE: Age + Gender + T + N + M + Overall Stage (6-dim)")
    print("Note: M stage is binarized (M0→0, M≥1→1), overall stage is ordinal+z-score")
    ...
    for fold in range(5):
        ...
        model = ClinicalNet(in_dim=X_tr.shape[1], n_bins=n_bins,
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            val_uno = _uno_from_logits(logits_full, bin_mids, y_tr_struct, y_va_struct, tau)
    ...
    print("\n" + "=" * 60)
    print("CLINICAL-ONLY SUMMARY: Age + Gender + T + N + M + Overall Stage")
    print("=" * 60)
