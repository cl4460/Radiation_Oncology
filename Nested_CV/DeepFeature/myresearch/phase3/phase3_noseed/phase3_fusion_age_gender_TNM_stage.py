#!/usr/bin/env python
# -*- coding: utf-8 -*-
# phase3_fusion_age_gender_TNM_stage.py
# Image embedding (frozen) + Clinical residual (Age + Gender + T + N + M + Overall Stage)
...
def build_clinical_features(
    ...
    """
    6 维特征: age + gender + T + N + M + overall_stage
    """
...
class ClinicalNet(nn.Module):
    """
    Clinical residual head: in_dim (=6) -> 32 -> n_bins
...
def main():
    ...
    print("\n" + "=" * 60)
    print("ADDITIVE FUSION: Age + Gender + T + N + M + Overall Stage (6-dim)")
    print("Note: M stage binarized, overall stage ordinal+z-score")
    ...
    for fold in range(5):
        ...
        in_dim = X_tr.shape[1]  # 应为 6
        clin_net = ClinicalNet(in_dim=in_dim, n_bins=n_bins,
                               dropout=cfg.CLIN_DROPOUT).to(DEVICE)
        ...
        optimizer = torch.optim.Adam(clin_net.parameters(),
    ...
    if fold_results:
        print("\n" + "=" * 60)
        print("SUMMARY: Age + Gender + T + N + M + Overall Stage")
        print("=" * 60)
