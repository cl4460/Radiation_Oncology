...
# phase3_imageonly.py
# Image-only survival training: CT + MASK (2-channel), ResNet18-GN, Logistic-Hazard
...
def safe_tau(times: np.ndarray, events: np.ndarray, q: float = 0.9, min_right: int = 5) -> float:
    """
    Pick a truncation time tau for Uno-IPCW that is less likely to violate
    P(D > tau) > 0 and less likely to be in extreme tail.  (scikit-survival requirement)
    """
    t = np.asarray(times, dtype=float)
    e = np.asarray(events, dtype=int)

    # Prefer event times if enough events; else use all observed times
    ev = t[e == 1]
    base = ev if ev.size >= 5 else t

    tau = float(np.quantile(base, q))
    mx = float(np.max(t))

    # avoid tau hitting the max follow-up exactly
    if tau >= mx:
        tau = mx - 1e-6
    if tau <= 0:
        tau = max(mx * 0.5, 1e-6)

    # ensure there are at least a few subjects with follow-up > tau (avoid degenerate tail)
    # if not, move tau left by decreasing quantile.
    qq = q
    while int((t > tau).sum()) < min_right and qq > 0.5:
        qq -= 0.05
        tau = float(np.quantile(base, qq))
        if tau >= mx:
            tau = mx - 1e-6

    return tau

def uno_or_censored(y_tr_struct, y_va_struct, risk: np.ndarray, tau: float) -> float:
    """
    Try Uno-IPCW; if it fails or returns NaN/inf, fallback to Harrell's C-index (censored).
    """
    r = np.asarray(risk, dtype=np.float64)
    r = np.nan_to_num(r, nan=0.0, posinf=1e6, neginf=-1e6)  # defensive

    try:
        v = float(concordance_index_ipcw(y_tr_struct, y_va_struct, r, tau=tau)[0])
        if (not np.isfinite(v)) or np.isnan(v):
            raise ValueError(f"Uno returned non-finite: {v}")
        return v
    except Exception as ex:
        print(f"[WARN] Uno-IPCW failed (tau={tau:.3f}) -> fallback to censored C-index. Reason: {ex}")
        return float(concordance_index_censored(
            y_va_struct["event"], y_va_struct["time"], r
        )[0])
...
# ================= Model =================
class ResNet18_ImageOnly_GN(nn.Module):
...
def run_fold(fold: int, manifest: pd.DataFrame, idx_tr, idx_va, out_dir: Path) -> float:
    ...
    model = ResNet18_ImageOnly_GN(in_channels=2, n_bins=n_time_bins, dropout=cfg.DROPOUT).to(device)
    ...
    tau = safe_tau(times[idx_tr], events[idx_tr], q=0.9, min_right=5)
    ...
    for ep in range(cfg.MAX_EPOCHS):
        ...
        uno_int = uno_or_censored(y_tr_struct, y_va_struct, risk_int, tau=tau)
        uno_24m = uno_or_censored(y_tr_struct, y_va_struct, risk_24m, tau=tau)
...
def main():
    ...
    exp = args.exp_name or f"imgonly_resnet18_lr{cfg.LR:.0e}" + ("_TTA" if cfg.USE_TTA else "_noTTA")
