#!/usr/bin/env python
# late_fusion_from_frozen_head.py
"""
...
def make_inner_split(events: np.ndarray, val_frac: float = 0.2, seed: int = 42):
    """
    Deterministic stratified split on event indicator.
    Returns: inner_train_idx, inner_val_idx (indices into TRAIN set arrays)
    """
    ...
    n1 = max(1, int(round(len(idx1) * val_frac))) if len(idx1) > 0 else 0
    n0 = max(1, int(round(len(idx0) * val_frac))) if len(idx0) > 0 else 0

    val_idx = np.concatenate([idx1[:n1], idx0[:n0]]) if (n1 + n0) > 0 else np.array([], dtype=int)
...
def build_clinical_features_for_fold(df: pd.DataFrame,
    ...
    # N z (no fill per your assumption; assert no NaN)
    ...
    # M z (no fill per your assumption; assert no NaN)
...
def main():
    ...
    for fold in range(5):
        ...
        # ---------------- NEW: pick alpha on INNER split of train ----------------
        inner_tr_idx, inner_va_idx = make_inner_split(
            e_tr,
            val_frac=0.2,
            seed=args.seed + 1000 + fold
        )

        y_inner_tr = to_struct(t_tr[inner_tr_idx], e_tr[inner_tr_idx])
        y_inner_va = to_struct(t_tr[inner_va_idx], e_tr[inner_va_idx])
        tau_inner = safe_tau(t_tr[inner_tr_idx], e_tr[inner_tr_idx])

        best_alpha, best_uno_inner = 1.0, -1.0
        alpha_curve = []

        for a in alphas:
            fused_inner = a * img_tr_n[inner_va_idx] + (1.0 - a) * clin_tr_n[inner_va_idx]
            try:
                uno_inner = float(concordance_index_ipcw(y_inner_tr, y_inner_va, fused_inner, tau=tau_inner)[0])
            except Exception:
                uno_inner = float(concordance_index_censored(
                    e_tr[inner_va_idx].astype(bool),
                    t_tr[inner_va_idx],
                    fused_inner
                )[0])
            alpha_curve.append((float(a), float(uno_inner)))
            if uno_inner > best_uno_inner:
                best_uno_inner = uno_inner
                best_alpha = float(a)

        # ---------------- EVAL on OUTER val using selected alpha (no leakage) ----------------
        fused_outer = best_alpha * img_va_n + (1.0 - best_alpha) * clin_va_n
        try:
            best_uno = float(concordance_index_ipcw(y_tr_st, y_va_st, fused_outer, tau=tau)[0])
        except Exception:
            best_uno = float(concordance_index_censored(e_va.astype(bool), t_va, fused_outer)[0])

        print(f"Selected alpha (inner): {best_alpha:.2f} | inner Uno: {best_uno_inner:.4f}")
        print(f"Outer-val fused Uno:   {best_uno:.4f}")
