...
def main():
    ...
    for fold in range(5):
        ...
        # ============ 2-feature Cox 融合（替换 grid search）============
        # 构建融合特征矩阵
        X_fusion_tr = np.column_stack([img_tr_n, clin_tr_n]).astype(np.float64)
        X_fusion_va = np.column_stack([img_va_n, clin_va_n]).astype(np.float64)

        # 融合 Cox（使用单独的正则参数，这里复用 cox_alpha，你也可以加 --fuse_alpha）
        cox_fuse = CoxPHSurvivalAnalysis(alpha=float(args.cox_alpha))
        cox_fuse.fit(X_fusion_tr, y_tr_st)

        # 预测融合 risk
        fused_risk_va = cox_fuse.predict(X_fusion_va).astype(np.float64)

        # 提取系数用于报告
        beta_img, beta_clin = cox_fuse.coef_
        alpha_rel = abs(beta_img) / (abs(beta_img) + abs(beta_clin) + 1e-12)

        # 评估
        try:
            best_uno = float(concordance_index_ipcw(y_tr_st, y_va_st, fused_risk_va, tau=tau)[0])
        except Exception:
            best_uno = float(concordance_index_censored(e_va.astype(bool), t_va, fused_risk_va)[0])

        # 打印（用于调试，开源前可删除）
        print(f"[FUSE-COX] beta_img={beta_img:.3f}, beta_clin={beta_clin:.3f}, alpha_rel={alpha_rel:.2f}")
        print(f"Fusion Uno: {best_uno:.4f}")
        ...
        print(f"Fused Uno (Cox):   {best_uno:.4f} (beta_img={beta_img:.3f}, beta_clin={beta_clin:.3f})")
