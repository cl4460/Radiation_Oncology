...
def load_and_merge(features_csv: Path,
    ...
    #    这里可以写死你R1/R2会用到的列名集合，或在run_r1里对num_cols做同样处理
    maybe_numeric = [
        "age",
        "clinical_T_Stage",
        "Clinical_N_Stage",
...
def run_r1(args: argparse.Namespace) -> None:
    ...
    for fold_name in fold_keys:
        ...
        if len(overlap) > 0:
            raise ValueError(f"[R1][{fold_name}] train/val overlap detected: {overlap[:10]} (n={len(overlap)})")
        print(f"[R1][{fold_name}] train/val split verified: no overlap, train={len(train_ids)}, val={len(val_ids)}")
        ...
        print(f"[R1][{fold_name}] Features: raw={len(feature_cols)} -> after_preprocess={n_features_after}")
        if n_features_after == 0:
            raise ValueError(f"[R1][{fold_name}] No features after preprocessing!")
        if n_features_after > args.warn_feature_dim_gt:
            print(f"[WARNING][R1][{fold_name}] Unexpectedly many features after preprocess: {n_features_after}")
        ...
        try:
            ...
            print(f"[WARNING][R1][{fold_name}] Could not extract coefficients: {e}")
    ...
    print(f"[R1] Overall OOF metrics (all folds combined):")
    print(f"     Harrell C (OOF): {summary['harrell_c_oof']}")
    print(f"     Uno C (OOF):     {summary['uno_c_oof']}")
    print(f"[R1] Outputs written to: {outdir.resolve()}")


def build_argparser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(description="Track R - R1: Clinical baseline (C_p) with CoxPHSurvivalAnalysis")
    ...
    ap.add_argument("--outdir", type=str, required=True, help="Output directory for R1.")
    ...
    ap.add_argument("--feature_cols", type=str, nargs="*", default=None,
                    help="Explicit raw feature cols for R1. If omitted, use [age, gender, Overall.Stage] if present; else fallback to all non-label cols.")
    ...
    ap.add_argument("--warn_feature_dim_gt", type=int, default=50,
                    help="Warn if features after preprocess exceed this number (R1 should be small).")
...
#--outdir /home/lichengze/Research/DeepFeature/myresearch/main/R1/R1_base  
#--id_col PatientID  
#--time_col "Survival.time"  
#--event_col "deadstatus.event"  
#--feature_cols age gender Overall.Stage  
#--cat_cols gender Overall.Stage  
#--cox_alpha 1e-4  
#--tau_q 0.9  
#--seed 42
...
#--labels_csv /home/lichengze/Research/CNN_pipeline/NSCLC-Radiomics-Lung1.clinical-version3-Oct-2019.csv \
#--splits_json /home/lichengze/Research/DeepFeature/myresearch/main/R0/splits.json \
#--outdir /home/lichengze/Research/DeepFeature/myresearch/main/R2/S0/R2_all_update_1e-4 \
