...
def parse_args():
    ...
    parser.add_argument('--use_tta', action='store_true', default=True,
                       help='Enable 4-view test-time augmentation (H/W flip)')
    parser.add_argument('--backbone', type=str, default='resnet10',
                       choices=['resnet10', 'resnet18'],
                       help='ResNet backbone architecture (resnet10 or resnet18)')
...
def build_transforms(train: bool) -> Compose:
    ...
    if train:
        ...
        transforms += [
            ...
            # Shape after EnsureChannelFirstd: [C, D, H, W]
            # spatial_axis=1 → H axis (Height, 192)
            # spatial_axis=2 → W axis (Width, 128)
...
class ResNet10_Clinical_GN(nn.Module):
    def __init__(self, in_channels: int = 2, clinical_dim: int = 6,
                 n_bins: int = 15, dropout: float = 0.35, backbone_type: str = 'resnet10'):
        super().__init__()
        
        # Select ResNet backbone
        if backbone_type == 'resnet10':
            resnet_func = monai_resnet.resnet10
        elif backbone_type == 'resnet18':
            resnet_func = monai_resnet.resnet18
        else:
            raise ValueError(f"Unsupported backbone: {backbone_type}. Choose 'resnet10' or 'resnet18'")
        
        self.backbone = resnet_func(
            spatial_dims=3,
...
def predict_with_tta4(model, img_batch, clin_batch, device, n_time_bins, _debug_first_call=[True]):
    ...
    Shape verification:
    - img_batch: [B, C, D, H, W] = [4, 2, 160, 192, 128]
    - dims=[-2] → H axis (192)
    - dims=[-1] → W axis (128)
    """
    ...
    if _debug_first_call[0]:
        ...
        print(f"   Expected: [B=4, C=2, D=160, H=192, W=128]")
        print(f"   dims=[-2] flips axis {img_batch.ndim - 2} (H={img_batch.shape[-2]})")
        print(f"   dims=[-1] flips axis {img_batch.ndim - 1} (W={img_batch.shape[-1]})")
...
# Training Function
# ============================================================
def run_fold(fold: int, manifest: pd.DataFrame, idx_train, idx_val, output_dir: Path, backbone_type: str = 'resnet10') -> float:
    print(f"\n{'='*70}")
    print(f"FOLD {fold} | LR={cfg.LR:.2e} | Backbone={backbone_type} | TTA={'ON' if cfg.USE_TTA else 'OFF'}")
    print(f"{'='*70}")
    ...
    print(f"📊 Model: {backbone_type.upper()} + Clinical (GroupNorm)")
...
def main():
    ...
    backbone_suffix = f"_{backbone_type}"
    exp_name = args.exp_name or f"lr_{cfg.LR:.0e}{backbone_suffix}{tta_suffix}"
    ...
    for fold, (idx_train, idx_val) in enumerate(skf.split(np.zeros(len(manifest)), 
        ...
        best_uno = run_fold(fold, manifest, idx_train, idx_val, output_dir, backbone_type)
        results.append({
            "fold": fold, 
            "best_uno": best_uno, 
            "lr": cfg.LR,
