...
OUTPUT_DIR = "/home/lichengze/Research/CNN_pipeline/phase3_outputs/learning_rate/learning_rate_corrected/output"
...
def main():
    ...
    for fold, (idx_train, idx_val) in enumerate(skf.split(np.zeros(len(manifest)), 
        ...
        print(f"Generating indices for Fold {fold}...")
        fold_dir = output_dir / f"fold_{fold}"
        fold_dir.mkdir(parents=True, exist_ok=True)
        
        np.save(fold_dir / "train_idx.npy", idx_train)
        np.save(fold_dir / "val_idx.npy", idx_val)
        
        print(f"  > Saved train_idx.npy ({len(idx_train)} indices)")
        print(f"  > Saved val_idx.npy ({len(idx_val)} indices)")
