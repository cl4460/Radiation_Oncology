...
def main():
    ...
    for fold_dir in fold_dirs:
        ...
        # Determine which checkpoint to load based on selection.json
        selection_path = fold_dir / "selection.json"
        chosen_variant = None
        if selection_path.exists():
            try:
                sel = json.load(open(selection_path))
                chosen_variant = sel.get("chosen_variant", None)
            except Exception:
                pass
        
        # Try to load from chosen_variant/best.pt first, fallback to fold_dir/best.pt
        ckpt_path = None
        if chosen_variant:
            variant_ckpt_path = fold_dir / chosen_variant / "best.pt"
            if variant_ckpt_path.exists():
                ckpt_path = variant_ckpt_path
        
        # Fallback to fold_dir/best.pt if chosen_variant checkpoint doesn't exist
        ...
        # Get variant from checkpoint, or infer from chosen_variant or use_clinical
        variant = ckpt.get("variant", None)
        if variant is None or variant == "unknown":
            if chosen_variant:
                variant = chosen_variant
            elif use_clinical:
                variant = "model_imgclin"
            else:
                variant = "model_img"
